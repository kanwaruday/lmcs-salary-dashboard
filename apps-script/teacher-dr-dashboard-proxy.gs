// ═══════════════════════════════════════════════════════════════════
// Teacher DR Dashboard Proxy — read-only Web App in front of
// "LMCS Teacher DR 2026 (Responses)" (spreadsheet id
// 1cP-f8ShSJJPMBXkQCfkSvmajPA1db6fMBOPCbLH3oIc), the response sheet
// for the 6 campus "LMS N Teacher DR (Qualitative) 2026 for Heads"
// Google Forms.
//
...
// for the portal's Teaching DR dashboard, same "narrow read proxy in
...
//   3. Copy the Web App URL into teacher-dr/index.html's
//      DASHBOARD_API_URL constant
// ═══════════════════════════════════════════════════════════════════

const TDR_RESPONSES_SHEET_ID = '1cP-f8ShSJJPMBXkQCfkSvmajPA1db6fMBOPCbLH3oIc'; // "LMCS Teacher DR 2026 (Responses)"
const TDR_ALLOWLIST_SHEET_ID = '1NZu0ElismFytG395Nxjz29vAz7OfkmJtZhs70bOwT58'; // "LMCS Principal Allowlist"
const TDR_GOOGLE_CLIENT_ID = '697999989724-mvi85iobr20g4mm8a8nrjd1rms2o8tf6.apps.googleusercontent.com';

// campusId (as used by the portal, e.g. 'LMS1') -> the exact tab name
// in the responses sheet.
const TDR_CAMPUS_TO_TAB = {
...
// dr-forms-sync.gs's TEACHER_DR_RUBRIC_TITLES / the sheet's real
// headers exactly.
const TDR_RUBRIC_COLUMNS = [
...
function doGet(e) {
  try {
    const caller = verifyDRDashboardToken_(e.parameter.idToken);
    if (!caller) return tdrJsonOut_({ success: false, error: 'Not authorized' });

    const campuses = caller.campusId === 'ALL' ? Object.keys(TDR_CAMPUS_TO_TAB) : [caller.campusId];
    const byCampus = {};
    campuses.forEach(function (campusId) {
      byCampus[campusId] = readCampusStats_(campusId);
    });
    return tdrJsonOut_({ success: true, rubricParams: TDR_RUBRIC_COLUMNS, campuses: byCampus });
  } catch (err) {
    return tdrJsonOut_({ success: false, error: err.message });
  }
}
...
function verifyDRDashboardToken_(idToken) {
  ...
  try {
    ...
    if (payload.aud !== TDR_GOOGLE_CLIENT_ID) return null;
    if (payload.email_verified !== 'true' && payload.email_verified !== true) return null;
    const email = (payload.email || '').toLowerCase();

    const rows = SpreadsheetApp.openById(TDR_ALLOWLIST_SHEET_ID).getSheets()[0].getDataRange().getValues();
...
function readCampusStats_(campusId) {
  const tabName = TDR_CAMPUS_TO_TAB[campusId];
  const sheet = SpreadsheetApp.openById(TDR_RESPONSES_SHEET_ID).getSheetByName(tabName);
  ...
  const rubricCols = TDR_RUBRIC_COLUMNS.map(function (title) { return header.indexOf(title); });
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    if (!byTeacher[teacher]) {
      byTeacher[teacher] = { count: 0, sumTotal: 0, sumPerParam: TDR_RUBRIC_COLUMNS.map(function () { return 0; }), lastDate: null };
    }
